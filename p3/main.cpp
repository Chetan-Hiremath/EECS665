...
static void usageAndDie(){
	std::cerr << "Usage: jeffc <infile> <options>\n"
...
static jeff::ProgramNode * syntacticAnalysis(const char * inFile){
	std::ifstream inStream(inFile);
	if (!inStream.good()){
		std::string msg = "Bad input stream ";
		msg += inFile;
...
static void doUnparsing(jeff::ProgramNode * ast, const char * outPath){
	if (outPath == nullptr){ 
...
main( const int argc, const char **argv )
{
	...
	if (unparseFile != nullptr){
		try {
			...
			std::cerr << "Something in the compiler is broken: " << e->msg() << std::endl;
			exit(1);
		} catch (ToDoError * e){
			std::cerr << "ToDo: " << e->msg() << std::endl;
			exit(1);
		} catch (UserError * e){
		std::cerr << "The user made a mistake: " << e->msg() << std::endl;
		exit(1);
