...
void IndexNode::typeAnalysis(TypeAnalysis * typing){
	id->typeAnalysis(typing);
	exp->typeAnalysis(typing);

	const DataType * baseType = typing->nodeType(id);
	const DataType * offType = typing->nodeType(exp);
	...
	bool validElt = true;
	if (asArr == nullptr){
		typing->errArrayID(exp->pos());
		typing->nodeType(this, ErrorType::produce());
	}

	if (offType->isInt()){
		if (asArr){
			const DataType * eltType = asArr->getEltType();
...
static bool validAssignOpd(const DataType * type){
	if (type->isBool() || type->isInt() || type->isFile()){
		return true;
	}
	if (type->asError()){
		return true;
...
void TernaryExpNode::typeAnalysis(TypeAnalysis * typing){
	bool isError = false;
	DataType * errType = ErrorType::produce();

	bool condOk = typeCondOpd(typing, cond);
	if (!condOk){
		isError = true;
	}

	expTrue->typeAnalysis(typing);
	auto typeTrue = typing->nodeType(expTrue);
	if (typeTrue->asError()){
		isError = true;
	}

	expFalse->typeAnalysis(typing);
	auto typeFalse = typing->nodeType(expFalse);
	if (typeFalse->asError()){
		isError = true;
	}

	if (!typeTrue->asError() && !typeFalse->asError()){
		if (!(typeTrue == typeFalse)){
			if (!isError){
				typing->errTernaryOpr(this->pos());
			}
			isError = true;
		} else if (!isError){
			typing->nodeType(this, typeTrue);
			return;
		}
	}

	typing->nodeType(this, errType);
}
...
void OpenStmtNode::typeAnalysis(TypeAnalysis * typing){
	this->handle->typeAnalysis(typing);
	const DataType * childType = typing->nodeType(this->handle);
	if (childType->asError()){
		return;
	} else if (childType->isFile()){
		return;
	}
	typing->errFileOpen(this->handle->pos());
}

void CloseStmtNode::typeAnalysis(TypeAnalysis * typing){
	this->handle->typeAnalysis(typing);
	const DataType * childType = typing->nodeType(this->handle);
	if (childType->asError()){
		return;
	...
	typing->errFileClose(this->handle->pos());
}

void ReadStmtNode::typeAnalysis(TypeAnalysis * typing){
	myDst->typeAnalysis(typing);
	myHandle->typeAnalysis(typing);
	const DataType * childType = typing->nodeType(myDst);
	const DataType * handleType = typing->nodeType(myHandle);

	if (handleType->asError()){
		typing->nodeType(this, ErrorType::produce());
	} else if (handleType->isFile()){
		typing->nodeType(this, BasicType::VOID());
	} else {
		typing->nodeType(this, BasicType::VOID());
	...
	if (childType->isBool()){
		...
		typing->nodeType(this, ErrorType::produce());
		return;
	} else if (childType->asArray()){
		typing->errReadArray(myDst->pos());
		typing->nodeType(this, ErrorType::produce());
		return;
	} else if (childType->isFile()){
...
void WriteStmtNode::typeAnalysis(TypeAnalysis * typing){
	...
	if (srcType->isVoid()){
		...
		typing->nodeType(this, ErrorType::produce());
		return;
	} else if (srcType->asArray()){
		typing->errOutputArray(mySrc->pos());
		typing->nodeType(this, ErrorType::produce());
		return;
